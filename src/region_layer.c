...
layer make_region_layer(int batch, int w, int h, int n, int classes, int coords,char *name,char *bottom_name,char *top_name)
{
    layer l = {0};
    l.type = REGION;
    ...
    l.inputs = l.outputs;
    l.truths = 30*(5);
    l.bottom_data = calloc(batch*l.inputs);
    ...
    l.bottom_data_gpu = cuda_make_array(l.bottom_data,batch*l.outputs);
    l.output_gpu = cuda_make_array(l.output, batch*l.outputs);
    l.delta_gpu = cuda_make_array(l.delta, batch*l.outputs);
    l.top_data = l.output_gpu;
#endif
...
void forward_region_layer(const layer l, network_state state)
{
    ...
    *(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
    printf("Cost:%f ,Region Avg IOU: %f, Class: %f, Obj: %f, No Obj: %f, Avg Recall: %f,  count: %d\n", *(l.cost),avg_iou/count, avg_cat/class_count, avg_obj/count, avg_anyobj/(l.w*l.h*l.n*l.batch), recall/count, count);
}
...
void forward_region_layer_gpu(const layer l, network_state state,int n )
{
    /*
       if(!state.train){
    ...
    flatten_ongpu(l.bottom_data_gpu, l.h*l.w, l.n*(l.coords + l.classes + 1), l.batch, 1, l.output_gpu);
    if(l.softmax_tree){
        int i;
        int count = 5;
