...
deconvolutional_layer make_deconvolutional_layer(int batch, int h, int w, int c, int n, int size, int stride, ACTIVATION activation,char*name,char* bottom_name,char* top_name)
{
    int i;
    ...
    l.top_data = l.output;
    l.bottom_data = calloc(l.batch*l.outputs,sizeof(float));
    ...
    l.bottom_data_gpu = cuda_make_array(l.bottom_data,l.inputs*l.batch);
    #endif
